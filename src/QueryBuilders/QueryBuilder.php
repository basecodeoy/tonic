...
namespace BaseCodeOy\Tonic\QueryBuilders;
        ...
        foreach ($this->queryRelationships as $relationshipResource => $relationships) {
            foreach ($relationships as $relationship) {
                if ($relationshipResource === 'self') {
                    $withs[$relationship] = fn (Builder|Relation $query): \Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Eloquent\Relations\Relation => $query;
        ...
        foreach ($this->queryFilters as $filterResource => $filters) {
            ...
            foreach ($filters as $filter) {
                if ($filterResource === 'self') {
                    ...
                    $filterRelationships[$filterResource] = fn (Builder|Relation $query): \Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Eloquent\Relations\Relation|\BaseCodeOy\Tonic\QueryBuilders\QueryBuilder => $this->applyFilter($query, $filter);
                }
            }
